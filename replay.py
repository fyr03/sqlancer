...
def run(args):
    ...
    for idx, sql in enumerate(statements, 1):
        if args.seed is not None:
            sql = re.sub(r'\bRAND\s*\(\s*\)', f'RAND({args.seed})', sql,
                        flags=re.IGNORECASE)
        try:
            cursor.execute(sql)
    ...
    # ── 子集检测：取最后两条 SELECT，构造 EXCEPT 查询 ──────────────────
    # oracle1: S1 ⊆ S2  →  违规行 = (q1) EXCEPT (q2)，即 S1 有但 S2 没有
    # oracle2: S2 ⊆ S1  →  违规行 = (q2) EXCEPT (q1)，即 S2 有但 S1 没有
    selects = [s for s in statements if s.strip().upper().startswith('SELECT')]
    if len(selects) >= 2:
        q1 = selects[-2].rstrip().rstrip(';')
        q2 = selects[-1].rstrip().rstrip(';')

        if args.oracle == 'oracle1':
            # S1 ⊆ S2：检测 S1 中有但 S2 没有的行
            except_sql  = f"({q1})\nEXCEPT\n({q2})"
            label       = 'S1 ⊆ S2'
            missing_msg = 'S1 有但 S2 没有'
        else:
            # S2 ⊆ S1：检测 S2 中有但 S1 没有的行
            except_sql  = f"({q1})\nEXCEPT\n({q2})"
            label       = 'S2 ⊆ S1'
            missing_msg = 'S2 有但 S1 没有'

        logger.write(f'\n-- [子集检测] {label} 验证 (EXCEPT 结果为空则关系成立)\n')
        try:
            cursor.execute(except_sql)
            rows = cursor.fetchall()
            cols = list(rows[0].keys()) if rows else []
            if not rows:
                logger.write(except_sql + '\n')
                logger.write(f'(empty) -- {label} 成立 ✓\n')
            else:
                logger.write(except_sql + '\n')
                logger.write(f'-- ✗ 违规！以下 {len(rows)} 行 {missing_msg}:\n')
                logger.log_select(except_sql, rows, cols)
        except MySQLError as e:
            logger.write(except_sql + '\n')
            logger.write(f'ERROR: {e}\n')
    else:
        logger.write('\n-- [子集检测] 未找到足够的 SELECT 语句，跳过\n')
...
def parse_args():
    ...
    p.add_argument('--oracle',   default='oracle1',
                   choices=['oracle1', 'oracle2'],
                   help='子集方向：oracle1=S1⊆S2（默认），oracle2=S2⊆S1')
    p.add_argument('--seed', default=None, type=int,
               help='固定 RAND() 的种子值（用于复现 bug，对应 SQLancer 输出的 Seed）')
    return p.parse_args()
