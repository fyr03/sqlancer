...
"""
mariadb_replay.py
读取包含 SQL 语句序列的 txt 文件，在 MariaDB 中逐条执行，
并将「执行的 SQL + 返回结果」输出到指定 log 文件。

用法：
    python mariadb_replay.py \
        --input  logs/mariadb/database3.log \
...
依赖：
    pip install mysql-connector-python  # MariaDB 兼容 MySQL 连接器
"""
...
def run(args):
    ...
    try:
        ...
        logger.log_init_error(f"无法连接 MariaDB 或初始化数据库：{e}")
        logger.close()
        sys.exit(1)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="SQLancer SQL 回放脚本：读取 SQL 文件并在 MariaDB 中执行，输出结构化日志"
    )
    p.add_argument('--input',    required=True,  help='输入的 SQL txt/log 文件路径')
    p.add_argument('--output',   required=True,  help='输出日志文件路径')
    p.add_argument('--host',     default='127.0.0.1', help='MariaDB 主机（默认 127.0.0.1）')
    p.add_argument('--port',     default=3309, type=int, help='MariaDB 端口（默认 3309）')
    p.add_argument('--user',     default='root', help='MariaDB 用户名（默认 root）')
    p.add_argument('--password', default='123456',     help='MariaDB 密码')
